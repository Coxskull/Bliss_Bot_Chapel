...
| ECO-BLISS | Bliss | Inventory rate intelligence | Phases 1–3 structured inputs | 🟡 inputs only | `docs/economics/PHASE-3-EVIDENCE.md` | Persistence/API/UI tests | Med if faked | Later phases gated | Do not hard-code CPM or 20/80 |
...
IMPLEMENTED Phases 1–3: EF entities/migrations for geographic markets,
pricing models, research sources, benchmark observations, and versioned
creator audience/performance snapshots, market/industry profiles,
inventory benchmarks, and dated FX; GET-only API; read-only operator
visibility; PostgreSQL evidence.

MISSING (later gated phases): recommendation engine, quotes,
pricing-rule execution, compensation rules, n8n research jobs, and AI
extraction with provenance writes.
